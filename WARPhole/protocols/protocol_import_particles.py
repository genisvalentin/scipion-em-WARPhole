...
class WARPholeImportParticles(EMProtocol):
    ...
    def _insertAllSteps(self):
        ...
        #importStep = self._insertFunctionStep('readNewMicrographs',self.starFile.get())
    ...
    def _stepsCheck(self):
        ...
        for particleSet in newParticleSetList:
            newSteps.append(
            self._insertFunctionStep('importMicrographStep',particleSet,prerequisites=[])
            )
        if len(newSteps)>0:
    ...
    def readNewMicrographs(self, filename, **kwargs):
        ...
        if self.streamingHasFinished():
            ...
            return(newParticleSetList[:-1])#We always throw away the last micrograph because it could be incomplete
